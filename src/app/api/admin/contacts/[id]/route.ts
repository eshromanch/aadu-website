...
async function authenticateRequest(req: NextRequest) {
  // During build, skip authentication
  if (isBuildProcess) {
    return null
  }
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  if (isBuildProcess) {
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Contact updated successfully (build mode)',
      data: { 
        _id: 'build-mode-id',
        name: 'Build Mode',
        email: 'build@example.com',
  ...
  try {
    ...
    if (updateData.adminNotes !== undefined) allowedUpdates.adminNotes = updateData.adminNotes
